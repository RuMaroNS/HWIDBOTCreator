...
// Главное меню (Добавлена кнопка создания апдейтов)
...
bot.on('text', async (ctx, next) => {
    ...
    if (userSessions[userId] === 'await_custom_graph') {
        if (!text.includes('/')) {
            return ctx.reply("❌ Ошибка! Отсутствует разделитель `/`. Попробуйте еще раз в формате `ГГГГ.ММ.ДД / Название подписки`.");
        }
        ...
        if (isNaN(expiresAt.getTime())) {
            return ctx.reply("❌ Неверный формат даты! Пример: `2026.05.20 / Окак`");
        }
    ...
    // 3. Логика создания апдейт лога (Вернул обратно)
    if (userSessions[userId] === 'await_update_log') {
        if (!text.includes('|')) {
            return ctx.reply('❌ Ошибка! Используйте черту `|` для разделения версии и описания. Попробуйте еще раз:');
        }
