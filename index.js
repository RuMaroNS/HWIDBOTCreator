...
// НАСТОЯЩИЙ СЛУЧАЙНЫЙ ГЕНЕРАТОР КЛЮЧЕЙ (Формат: xxxx-xxxx-xxxx-xxxx)
...
const mainMenu = Markup.keyboard([
    ...
    ['📋 Активные ключи HWID', '⚙️ Настройки бота']
]).resize();

// Список точных названий кнопок для проверки сброса сессий
const menuButtons = ['🔑 Создать ключ HWID', 'создать ключ HWID', '📝 Создать апдейт', '📋 Активные ключи HWID', 'активные ключи HWID', '⚙️ Настройки бота', 'Настройки бота'];

// Функция для сброса сессии пользователя
...
// Слушатель обычного текста с защитой от перехвата главных кнопок меню
bot.on('text', async (ctx, next) => {
    const userId = ctx.from.id;
    const text = ctx.message.text.trim();

    // Если отправленный текст — это кнопка меню, сбрасываем ожидания и передаем управление дальше
    ...
    if (userSessions[userId] === 'await_custom_graph') {
        if (!text.includes('/')) {
            return ctx.reply("❌ Ошибка! Отсутствует разделитель `/`.\nПришлите строку в формате: `ГГГГ.ММ.ДД / Название подписки` или нажмите любую кнопку меню для отмены.");
        }
        ...
        if (isNaN(expiresAt.getTime())) {
            return ctx.reply("❌ Неверный формат даты! Убедитесь, что дата реальна. Пример: `2026.06.10 / Тест` или нажмите кнопку меню для отмены.");
        }
    ...
    if (userSessions[userId] === 'await_update_log') {
        if (!text.includes('|')) {
            return ctx.reply('❌ Ошибка! Используйте черту `|` для разделения версии и описания.\nПример: `1.0.2 | Пофиксили баг` или нажмите кнопку меню для отмены:');
        }
...
bot.action('gen_custom_prompt', async (ctx) => {
    ...
    return ctx.editMessageText('⚙️ **Режим создания своего графика**\n\n' +
                               ...
                               '*Пример:* `2026.06.10 / Тест` \n\n' +
                               '_(При нажатии любой кнопки главного меню режим сбросится автоматически)_', 
                               { parse_mode: 'Markdown' });
});

// Нажатие: "⚙️ Настройки бота"
bot.hears(['⚙️ Настройки бота', 'Настройки бота'], async (ctx) => {
    if (!(await isAdmin(ctx.from.id, ctx.from.username))) return;
    clearSession(ctx.from.id);
    
    ctx.reply("⚙️ **Настройки управления ботом:**", Markup.inlineKeyboard([
...
bot.action("view_admins_list", async (ctx) => {
    ...
    admins.forEach((admin) => {
        text += `•👤 \`${admin.user_id}\` _(Добавил: ${admin.added_by || 'Система'})_\n`;
    });

    ctx.editMessageText(text, { parse_mode: 'Markdown', ...Markup.inlineKeyboard([[Markup.button.callback("⬅️ Назад в настройки", "back_to_settings")]]) });
});
...
bot.hears(['📋 Активные ключи HWID', 'активные ключи HWID'], async (ctx) => {
    if (!(await isAdmin(ctx.from.id, ctx.from.username))) return;
    clearSession(ctx.from.id);
...
async function sendKeysPage(ctx, page, isEdit = true) {
    ...
    pageKeys.forEach((k, index) => {
        ...
        inlineButtons.push([Markup.button.callback(`🔍 Посмотреть [${displayIdx}]`, `view_key_${k.id}_${page}`)]);
    });
...
async function renderSingleKey(ctx, keyId, backPage) {
    ...
    const actions = Markup.inlineKeyboard([
        [
            Markup.button.callback(blockButtonText, blockAction), 
            Markup.button.callback("🖥 Сбросить HWID", `reset_hwid_${k.id}_${backPage}`)
        ],
        [Markup.button.callback("🗑 Полностью удалить ключ", `delete_key_${k.id}_${backPage}`)],
